...
(function($) {
	...
	jQuery.fn.superbgTransition = function(thisimg, dimensions) {
		...
		// transition fadeIn
			...
			var pom = Math.floor(Math.random() * 10);
            var zoomType = parseInt(pom, 10);
            
            
            if(zoomType == 2 || zoomType == 5) {    
                $(thisimg).css({height: '+=150px', width: '+=150px', left: '-=75px', top: '-=75px'});
            }
            else if(zoomType == 3 || zoomType == 7) {
                $(thisimg).css({height: '+=150px', width: '+=150px', left: '-=150px', top: '-=150px'});
            }
            else if(zoomType == 8) {
                $(thisimg).css({height: '+=150px', width: '+=150px'});
            }
            ...
            if(options.zoomEfect === 1)
            {
                //udelame ruzne typy zoomu
                if(zoomType == 0 || zoomType == 4)
                {
                    $(thisimg).animate({
                        height: '+=150px', width: '+=150px', left: '-=75px', top: '-=75px'
                      }, {queue: false, duration: options.zoomEfect_interval}, function() {
                        
                        setTimeout(function() {
                          $(thisimg).css({height: '-=150px', width: '-=150px', left: '+=75px', top: '+=75px'});
                        }, (options.zoomEfect_interval - options.speed));
                        
                        //$(thisimg).delay(6000).animate({height: '-=150px', width: '-=150px', left: '+=75px', top: '+=75px'},options.zoomEfect_interval);
                      });
                }
                else if(zoomType == 6) {
                    $(thisimg).animate({
                        ...
                        //$(thisimg).delay(6000).animate({height: '-=150px', width: '-=150px'},options.zoomEfect_interval);
                        setTimeout(function() {
                          $(thisimg).css({height: '-=150px', width: '-=150px'});
                        }, (options.zoomEfect_interval - options.speed));
                      });
                }
                else if(zoomType == 1 || zoomType == 9) {
                    $(thisimg).animate({
                        height: '+=150px', width: '+=150px', left: '-=150px', top: '-=150px'
                      }, {queue: false, duration: options.zoomEfect_interval}, function() {
                        //$(thisimg).delay(6000).animate({height: '-=150px', width: '-=150px', left: '+=150px', top: '+=150px'},options.zoomEfect_interval);
                        setTimeout(function() {
                          $(thisimg).css({height: '-=150px', width: '-=150px', left: '+=150px', top: '+=150px'});
                        }, (options.zoomEfect_interval - options.speed));
                      });
                }
                else if(zoomType == 2 || zoomType == 5) {
                    //$(thisimg).css({height: '+=150px', width: '+=150px', left: '-=75px', top: '-=75px'});
                    $(thisimg).animate({height: '-=150px', width: '-=150px', left: '+=75px', top: '+=75px'},{queue: false, duration: options.zoomEfect_interval});
                }
                else if(zoomType == 3 || zoomType == 7) {
                    //$(thisimg).css({height: '+=150px', width: '+=150px', left: '-=150px', top: '-=150px'});
                    $(thisimg).animate({height: '-=150px', width: '-=150px', left: '+=150px', top: '+=150px'},{queue: false, duration: options.zoomEfect_interval});
                }
                else {
                    //$(thisimg).css({height: '+=150px', width: '+=150px', left: '+=150px', top: '+=150px'});
                    $(thisimg).animate({height: '-=150px', width: '-=150px'},{queue: false, duration: options.zoomEfect_interval});
                }
	...
	jQuery.fn.superbgimage.defaults = {
        ...
        zoomEfect: 0, //animation zoom efect, 0-none, 1-zoom efect (zoomIn/Out)
        zoomEfect_interval: 2000 //time zoomIn and after time zoomOut - best half slide_interval
	};

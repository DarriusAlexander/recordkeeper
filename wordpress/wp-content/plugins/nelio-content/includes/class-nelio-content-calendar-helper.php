 ...
 * @since      1.4.2
 */
class Nelio_Content_Calendar_Helper {
	 ...
	 * @since  1.4.2
	 * @access protected
	 * @var    Nelio_Content_Calendar_Helper
	 */
	 ...
	 *
	 * @return Nelio_Content_Calendar_Helper the single instance of this class.
	 *
	 * @since  1.4.2
	 * @access public
	 */
	...
	public function handle_ics_subscription() {
		...
		$this->start_date = apply_filters( 'nelio_content_calendar_ics_subscription_start_date', $this->get_beginning_of_week( date( 'Y-m-d', current_time( 'timestamp' ) ) ) );
		$this->total_weeks = apply_filters( 'nelio_content_calendar_total_weeks', 6, 'ics_subscription' );
	...
	private function get_calendar_posts_for_week( $args = array() ) {
		...
		$args = apply_filters( 'nelio_content_calendar_posts_query_args', $args );
	...
	public function posts_where_week_range( $where = '' ) {
		...
		$calendar_helper = Nelio_Content_Calendar_Helper::instance();
	...
	private function get_post_information_fields( $post ) {
		...
		$information_fields = apply_filters( 'nelio_content_calendar_item_information_fields', $information_fields, $post->ID );
		foreach ( $information_fields as $field => $values ) {
			...
			if ( ( apply_filters( 'nelio_content_calendar_hide_empty_item_information_fields', true, $post->ID ) && empty( $values['value'] ) )
					|| apply_filters( "nelio_content_calendar_hide_{$field}_item_information_field", false, $post->ID ) ) {

...
class Nelio_Content_Notifications {
	...
	public function define_admin_hooks() {
		...
		add_action( 'plugins_loaded', array( $this, 'maybe_add_notification_hooks' ) );
	 ...
	 * @access public
	 */
	public function maybe_add_notification_hooks() {
	...
	}//end maybe_add_notification_hooks()
	...
	private function get_task_completed_email_data( $task, $post ) {
		...
		if ( $post ) {
			...
			/* translators: blog name */
			$subject = sprintf( _x( '[%s] Editorial Task Completed', 'text', 'nelio-content' ), $blog_name );
	...
	private function get_task_information( $task ) {
		...
		if ( $task->date_due ) {
			...
			$info .= " - " . sprintf( _x( 'Due Date: %1$s', 'text', 'nelio-content' ), $task_due_date, $task_due_time ) . "\r\n";
		}//end if
	...
	private function get_email_footer( $post = false ) {
		...
		if ( $post ) {
			...
			$footer .= sprintf( _x( '%1$s: %2$s', 'command', 'nelio-content' ), $post_type_labels->edit_item, $edit_link ) . "\r\n";
			$footer .= sprintf( _x( '%1$s: %2$s', 'command', 'nelio-content' ), $post_type_labels->view_item, $view_link ) . "\r\n";

...
class SunshineCart {
	...
	*/
		...
		if ( !empty( $this->content ) ) {
			...
			if ( is_array( $auto_discounts ) ) {
				foreach( $auto_discounts as $auto_discount ) {
					...
					if ( !in_array( $auto_discount->ID, $this->discounts ) ) {
	...
	public function set_subtotal() {
		...
		$this->subtotal = apply_filters( 'sunshine_subtotal', $subtotal );
	}
	...
	public function set_shipping() {
		...
		if ( !empty( $this->shipping_method['id'] ) ) {
			$this->shipping_method['cost'] = apply_filters( 'sunshine_shipping_set_shipping_cost', $this->shipping_method['cost'], $this->shipping_method['id'], $this );
		}
			...
			switch ( $discount->discount_type ) {
				case 'percent-total':
					if ( $discount->before_tax == 1 )
						...
						$this_discount = ( $this->subtotal + $this->tax ) * ( $discount->amount / 100 );

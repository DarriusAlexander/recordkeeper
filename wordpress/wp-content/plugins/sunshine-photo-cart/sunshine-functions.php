	...
	if ( isset( $_POST['sunshine_update_cart'] ) && $_POST['sunshine_update_cart'] == 1  && wp_verify_nonce( $_POST['nonce'], 'sunshine_update_cart' ) ) {
		...
		if ( !empty( $sunshine->cart->discounts ) ) {
			...
			foreach ( $sunshine->cart->discounts as $discount_id ) {
				...
				// Check max uses
				$use_count = get_post_meta( $discount_id, 'use_count', true );
				$max_uses = get_post_meta( $discount_id, 'max_uses', true );
				if ( !$sunshine->cart->discount_valid_max_uses( $use_count, $max_uses ) ) {
					$sunshine->cart->remove_discount( $discount_id );
					break;
				}
				...
				if ( !$sunshine->cart->discount_valid_max_uses_per_person( $code, $max_uses_per_person ) ) {
					$sunshine->cart->remove_discount( $discount_id );
					break;
				}
...
function sunshine_checkout_update_totals() {
	...
	if ( isset( $_POST['shipping_method'] ) ) {
		...
		$return['shipping'] = sunshine_money_format( $sunshine->shipping->get_shipping_method_cost( $sunshine->cart->shipping_method['id'] ), false );
	}
